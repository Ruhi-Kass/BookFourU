...
// ============================================================
// CHATBOT (With Interactive Category Menu)
// ============================================================
function initChatbot() {
    ...
    // 1. Function to display standard text messages
    ...
    // 2. The Core Chat Logic (Now accepts direct button clicks)
    async function handleChat(buttonText = null) {
        // Use the button text if clicked, otherwise use what they typed
        const message = buttonText || chatInput.value.trim();
        if (!message) return;
        ...
        const currentPageContext = document.title || "BOOK4U Website";

        try {
            const res = await fetch('/chat', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ message: message, context: currentPageContext })
            });
            const data = await res.json();
    ...
    // 3. NEW: The Interactive Greeting Menu
    function showInteractiveGreeting() {
        const msgHTML = `
            <div style="display:flex;gap:10px;justify-content:flex-start;margin-bottom:10px;">
                <div style="font-size:24px;">🤖</div>
                <div style="background:white;padding:12px 16px;border-radius:0 15px 15px 15px;box-shadow:0 2px 5px rgba(0,0,0,0.05);color:#333;font-size:14px;line-height:1.5;max-width:80%;">
                    Welcome to BOOK4U! 👋 What kind of books are you interested in today?
                    
                    <div style="margin-top: 12px; display: flex; flex-wrap: wrap; gap: 8px;">
                        <button class="chat-cat-btn" style="padding: 6px 12px; border: 1px solid #667eea; border-radius: 20px; background: white; color: #667eea; cursor: pointer; font-size: 12px; transition: 0.2s;">Educational</button>
                        <button class="chat-cat-btn" style="padding: 6px 12px; border: 1px solid #667eea; border-radius: 20px; background: white; color: #667eea; cursor: pointer; font-size: 12px; transition: 0.2s;">Fiction</button>
                        <button class="chat-cat-btn" style="padding: 6px 12px; border: 1px solid #667eea; border-radius: 20px; background: white; color: #667eea; cursor: pointer; font-size: 12px; transition: 0.2s;">Non-Fiction</button>
                        <button class="chat-cat-btn" style="padding: 6px 12px; border: 1px solid #667eea; border-radius: 20px; background: white; color: #667eea; cursor: pointer; font-size: 12px; transition: 0.2s;">anime mangas</button>
                    </div>
                </div>
            </div>`;
        chatMessages.insertAdjacentHTML('beforeend', msgHTML);
        ...
        const buttons = chatMessages.querySelectorAll('.chat-cat-btn');
        buttons.forEach(btn => {
            // Add a little hover effect using JS
            btn.addEventListener('mouseover', () => { btn.style.background = '#667eea'; btn.style.color = 'white'; });
            btn.addEventListener('mouseout', () => { btn.style.background = 'white'; btn.style.color = '#667eea'; });
            
            // What happens when clicked
            btn.addEventListener('click', function() {
                const category = this.innerText;
                // Instantly send a message asking for that category
                handleChat(`Show me your ${category} books`);
                
                // Hide or disable buttons after clicking so the chat stays clean
                buttons.forEach(b => b.style.display = 'none'); 
            });
        });
    }

    // Run the greeting if chat is empty
    if (chatMessages.innerHTML.trim() === '') {
        showInteractiveGreeting();
    }

    // Standard Enter key and Send button triggers
